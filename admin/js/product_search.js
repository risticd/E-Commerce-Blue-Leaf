    $(document).ready(function(){
        /*
         ...
         in the table.
        */
        $('#productResults').html('<p style="padding:5px;">Search Results</p>');
        ...
        $('#product-search').keyup(function() {
            ...
            if(searchVal !== '') {
                ...
                $.get('data/product_search.php?product-search='+searchVal, function(returnData) {
                    /* If the returnData is empty then display message to user
                     * else our returned data results in the table.  */
                    if (!returnData) {
                        $('#productResults').html('<p style="padding:5px;">No results found</p>');
                    } else {
                        $('#productResults').html(returnData);
                    }
                });
            } else {
                $('#productResults').html('<p style="padding:5px;">Search Results</p>');
