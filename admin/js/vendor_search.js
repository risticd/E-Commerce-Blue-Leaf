    $(document).ready(function(){
        /*
         ...
         in the table.
        */
        $('#vendorResults').html('<p style="padding:5px;">Search Results</p>');
        ...
        $('#vendor_search').keyup(function() {
            ...
            if(searchVal !== '') {
                ...
                $.get('data/vendor_search.php?vendor_search='+searchVal, function(returnData) {
                    /* If the returnData is empty then display message to user
                     * else our returned data results in the table.  */
                    if (!returnData) {
                        $('#vendorResults').html('<p style="padding:5px;"> No records found.</p>');
                    } else {
                        $('#vendorResults').html(returnData);
                    }
                });
            } else {
                $('#vendorResults').html('<p style="padding:5px;">Search Results</p>');
