    $(document).ready(function(){
        /*
         ...
         in the table.
        */
        $('#results').html('<p style="padding:5px; width:200px">Search by product number</p>');

        /* When the user enters a value such as "j" in the search box
         * we want to run the .get() function. */
        $('#outstandingBal').keyup(function() {
            ...
            if(searchVal !== '') {
                ...
                $.get('data/outstanding_search.php?outstandingBal='+searchVal, function(returnData) {
                    /* If the returnData is empty then display message to user
                     * else our returned data results in the table.  */
                    if (!returnData) {
                        $('#results').html('<p style="padding:5px; width:200px">No results found</p>');
                    } else {
