    $(document).ready(function(){
        /*
         ...
         in the table.
        */
        $('#adminResults').html('<p style="padding:5px;">Search Results</p>');
        ...
        $('#admin_search').keyup(function() {
            ...
            if(searchVal !== '') {
                ...
                $.get('data/admin-search.php?admin_search='+searchVal, function(returnData) {
                    /* If the returnData is empty then display message to user
                     * else our returned data results in the table.  */
                    if (!returnData) {
                        $('#adminResults').html('<p style="padding:5px;">No results found</p>');
                    } else {
                        $('#adminResults').html(returnData);
                    }
                });
            } else {
                $('#adminResults').html('<p style="padding:5px;">Search Results</p>');
