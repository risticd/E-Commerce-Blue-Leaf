    $(document).ready(function(){
        /*
         ...
         in the table.
        */
        $('#results').html('<p style="padding:5px; width:300px">Search by product number</p>');

        /* When the user enters a value such as "j" in the search box
         * we want to run the .get() function. */
        $('#topselling').keyup(function() {
            ...
            if(searchVal !== '') {
                ...
                $.get('data/topselling_search.php?topselling='+searchVal, function(returnData) {
                    /* If the returnData is empty then display message to user
                     * else our returned data results in the table.  */
                    if (!returnData) {
                        $('#results').html('<p style="padding:5px; width:300px">No results found</p>');
                    } else {
