    $(document).ready(function(){
        /*
         ...
         in the table.
        */
        $('regResults').html('<p style="padding:5px;">Search by product number</p>');

        /* When the user enters a value such as "j" in the search box
         * we want to run the .get() function. */
			 $('#regular-search').keyup(function() {
            ...
            if(searchVal !== '') {
                ...
                $.get('data/reg_order_search.php?regular-search='+searchVal, function(returnData) {
                    /* If the returnData is empty then display message to user
                     * else our returned data results in the table.  */
                    if (!returnData) {
                        $('#regResults').html('<p style="padding:5px;">No results found</p>');
                    } else {
                        $('#regResults').html(returnData);
                    }
                });
            } else {
                $('#regResults').html('<p style="padding:5px;">Search by product number</p>');
