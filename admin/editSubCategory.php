...
<input type="hidden" name="sub_category_id" value="<?php echo $id; ?>"/>
   ...
   <tr><td colspan="2" class="nohightlight"><h4>Please edit Category</h4></td></tr>
         ...
         <td class="nohightlight"><input type="text" name="sub_category_name" value="<?php echo $category ?>"></td>
      </tr>
	  <tr>
	  ...
	  <tr>
	  	 <td class="nohightlight"><input type="button" title="Add Sub Category" value="Add Sub Category" onclick="window.location = 'AddSubCategory.php';">
	  </tr>
...
<?php
 ...
 if (is_numeric($_POST['sub_category_id']))
 {
 // gets the form data and making sure it's valid
 $id = $_POST['sub_category_id'];
 $category = mysql_real_escape_string(htmlspecialchars($_POST['sub_category_name']));



 // save the data to the database
 mysql_query(" UPDATE sub_category
    SET sub_category_name='$category' WHERE sub_category_id=$id")
 or die(mysql_error()); 
 ...
 if (isset($_GET['sub_category_id']) && is_numeric($_GET['sub_category_id']) && $_GET['sub_category_id'] > 0)
 {
 // selecting all the field and displaying them into the textboxes
 $id = $_GET['sub_category_id'];
 $result = mysql_query("SELECT sub_category_name FROM sub_category WHERE sub_category_id=$id")
 or die(mysql_error()); 
...
$result = mysql_query($sql);
        ...
        echo "<tr> 
		<th colspan='3' title='sort' class='ui-state-default'><a href='#'>Sub Category</a></th></tr> 
		";
