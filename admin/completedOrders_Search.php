...
<?php
    ...
    if ($con) {
		...
		$fetch = mysql_query("SELECT account.username, customer_order_product.fk_customerorder_id, customer_order.customer_order_date, customer_order_product.date_required, customer_order_product.customer_order_status, customer_order_product.total_price FROM customer_order_product, customer_order, account WHERE customer_order_product.fk_customerorder_id = customer_order.customer_order_id AND customer_order.fk_account_id=account.account_id AND date_required REGEXP '^$query' AND customer_order_date REGEXP '^$query2' AND customer_order_product.customer_order_status = 'completed' ORDER BY customer_order_product.total_price DESC ");
